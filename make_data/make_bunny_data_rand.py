...
def bunny() :
    ...
    z = []
    i = 0
    for line in open('./make_data/bun_zipper.xyz', 'r') :
        i += 1
        tx, ty, tz, r, g, b = map(float, line.split())
        x.append(tx)
        y.append(ty)
        z.append(tz)
        if i == n :
    ...
    xyz = np.array([x, y, z])
    xyz[0] = (xyz[0] - np.sum(xyz[0]) / n) * 1000
    xyz[1] = (xyz[1] - np.sum(xyz[1]) / n) * 1000
    xyz[2] = (xyz[2] - np.sum(xyz[2]) / n) * 1000
    xyz_noised = xyz + np.random.normal(0, 3, (3, 1000)) # custom random parameter
...
def draw(xyz) :
    ...
    ax.set_aspect("equal")
    ax.scatter(xyz[0], xyz[1], xyz[2], color="g")
    plt.show()
...
file_path = "./init/initial_data_bunny_rand.txt" # custom output path
f = open(file_path, "w")

xyz, xyz_noised = bunny()
row, col = xyz.shape
f.write("%.i\n" % (col))
...
for th in ths :
    ...
    n = np.random.normal(0, 10, (3)) # custom random parameter
    n = n / np.linalg.norm(n) 
    phi = random.gauss(0, 0.01) # custom random parameter
    t = np.random.normal(0, 10, (3)) # custom random parameter
